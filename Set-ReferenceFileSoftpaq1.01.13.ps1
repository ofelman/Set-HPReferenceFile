...
$ReFileVersion = '1.01.13 Oct-28-2022'
...
Function Get_ReferenceFileArg {
    ...
    $f_CachingFolder = $pCacheDir+'\cache'
    $f_DestinationXmlFile = $pCacheDir+'\'+(Split-Path $pReferenceFile -Leaf)       # Destination path

    # if Referece File Argument is already in Current Folder, nothing to do
    ...
    if ( (Join-Path $f_DestinationXmlFile '') -eq (Join-Path $pReferenceFile '') ) {
        ...
        Try {
            ...
            if ( Test-Path $pReferenceFile ) {
                # first, we make sure the caching folder exists - as a directory, or we create one
                if ( (Test-Path $f_CachingFolder) -and (-not (Test-Path $f_CachingFolder -PathType Container)) ){ Remove-Item $f_CachingFolder }
                if ( -not (Test-Path $f_CachingFolder) ) { new-Item -Path $f_CachingFolder -itemtype Directory -Force }
                # next, we'll cache the Reference File passed as argument to the 'cacheing' folder
                Copy-Item $pReferenceFile -Destination $f_CachingFolder -Force -EA Stop
                # and, make a backup (one) if destination file already exists
                if ( Test-Path $f_DestinationXmlFile) {
                    Move-Item -Path $f_DestinationXmlFile -Destination $f_DestinationXmlFile'.bak' -Force -EA Stop
                }
                # finally, get the argument file to the current folder for use
                $f_CachedReferenceFile = $f_CachingFolder+'\'+(Split-Path $pReferenceFile -Leaf) 
